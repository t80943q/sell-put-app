...
st.title("🚀 Sell Put 智能量化终端 (中小资金特供版)")

# ================= 侧边栏风控设置 =================
st.sidebar.header("⚙️ 筛选风控与预算")
min_price = st.sidebar.number_input("最低股价 ($)", value=2.0, step=1.0)
max_price = st.sidebar.number_input("最高股价 ($)", value=200.0, step=10.0)
...
# 🌟 精简定制股票池：七姐妹(3只) + ETF(3只) + 中概股(3只) + 中小资金黄金标的
target_pool = [
    # 1. 🆕 中概股专区
    "XPEV",  # 小鹏汽车
    "BABA",  # 阿里巴巴
    ...
    try:
      ...
      for expiry in expirations[:3]:
        ...
        otm_puts["推荐挂单价"] = np.where(
            otm_puts["bid"] > 0,
            np.minimum(otm_puts["Mid"], otm_puts["bid"] * 1.2),
            otm_puts["lastPrice"],
        )
